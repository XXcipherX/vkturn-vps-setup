...
Если хочешь запускать WDTT как Docker Compose stack, по аналогии с `xray-vps-setup`, используй отдельный установщик:
...
Он установит Docker при необходимости, соберет `wdtt-server` из upstream-исходников с Go `1.26.4` и запишет stack в:

```text
/opt/vkturn-vps-setup/docker-compose.yml
/opt/vkturn-vps-setup/.env
/opt/vkturn-vps-setup/build/Dockerfile
/opt/vkturn-vps-setup/run-wdtt.sh
/opt/vkturn-vps-setup/data/
```

В репозитории для Docker-режима намеренно нет отдельного `Dockerfile`: `vps-setup.sh` генерирует его в `/opt/vkturn-vps-setup/build`, потому что публичного готового `wdtt-server` image сейчас нет и бинарник нужно собрать из Go-исходников.
...
Обычный `install.sh` с `systemd` остается основным вариантом для чистого VPS. Docker-вариант удобен, если тебе привычнее compose-структура и обновление через rebuild контейнера.
